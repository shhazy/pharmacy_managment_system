...
from ..models import Invoice, Batch, Product, User
...
def get_profit_data(db: Session = Depends(get_db_with_tenant)):
    results = db.execute(text("""
        SELECT p.product_name, SUM(ii.quantity * (ii.unit_price - b.purchase_price)) as profit
...
def slow_moving(db: Session = Depends(get_db_with_tenant)):
    results = db.execute(text("""
        SELECT p.product_name, SUM(b.current_stock) as stock
        FROM products p
...
def get_expiry_alerts(db: Session = Depends(get_db_with_tenant)):
    ...
    return db.query(Batch).join(Product).filter(Batch.expiry_date <= next_90_days, Batch.current_stock > 0).all()
